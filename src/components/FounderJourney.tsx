...
/* ThreeUI (Meng To's open-source three.js UI library) — canvas components, no CSS needed.
   Both auto-pause offscreen and are loaded client-side only to keep the initial bundle lean. */
...
export default function FounderJourney() {
  ...
  return (
    <section
      id="founder"
      className="relative py-24 lg:py-32 bg-[#F0F5FF]/75 backdrop-blur-md overflow-hidden"
    >
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          <p className="eyebrow mb-4">A message from the founder</p>
          <h2 className="text-[34px] sm:text-[40px] lg:text-[48px] font-bold text-[#1A2332] tracking-[-0.02em] leading-[1.05] max-w-3xl">
            From vision to a business that{' '}
            <span className="text-gradient">outlasts decades</span>
          </h2>
          <p className="mt-4 text-[16px] text-[#5A6B82] max-w-xl leading-relaxed">
            Every brand we take on walks this path with us. From the first
            conversation to a product that keeps winning for years. Step by
            step, this is how we help.
          </p>
        </motion.div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 px-4 py-3 lg:py-3.5 rounded-full border min-h-[44px] transition-all duration-300 ${
                    isActive
                      ? 'bg-[#1B2B5E] border-[#1B2B5E] text-[#EEF2FF] shadow-[0_8px_24px_rgba(27,43,94,0.35)]'
                      : 'bg-white border-[#C8D6E5] text-[#5A6B82] hover:border-[#7BB8E8] hover:text-[#1B2B5E]'
                  }`}
                >
                  <span
                    className={`font-mono text-[10px] tracking-widest ${
                      isActive ? 'text-[#7BB8E8]' : 'text-[#4A82C4]'
                    }`}
                  >
                    {s.number}
                  </span>
        ...
        <motion.div
          ...
          className="relative rounded-3xl overflow-hidden bg-[#101F45] border border-white/10 shadow-[0_24px_80px_rgba(16,31,69,0.35)]"
        >
          {/* ThreeUI stream backdrop — every step converges on your outcome */}
          <div className="absolute inset-0" aria-hidden="true">
            {reduced ? (
              <div className="absolute inset-0 bg-[#101F45]" />
            ) : (
              <StreamVisual
                hue={-35}
                saturation={0.85}
            ...
            {/* Brand tint pulls the violet streams into deep navy/blue */}
            <div className="absolute inset-0 bg-[#1B2B5E]/45" />
            <div className="absolute inset-0 bg-grid-dark opacity-50" />
            <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-[#101F45]/90 to-transparent" />
            <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[#101F45] to-transparent" />
          </div>
        ...
        {/* Bottom strip: orbit visual + the Hoardy promise */}
        <div className="mt-10 grid lg:grid-cols-5 gap-6">
          {/* ThreeUI orbital sphere — one team in orbit around your business */}
          <div className="lg:col-span-3 relative rounded-3xl overflow-hidden border border-[#C8D6E5] bg-white min-h-[380px] lg:min-h-[400px] flex flex-col">
            <div className="absolute inset-0" aria-hidden="true">
              {reduced ? (
                <div className="absolute inset-0 bg-gradient-to-br from-[#E2E9F5] to-[#F0F5FF]" />
              ) : (
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#101F45]/25 via-transparent to-transparent" />
              <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-white/70 to-transparent" />
            </div>
            <div className="relative p-8 lg:p-10">
              <p className="eyebrow mb-3">Your business at the center</p>
              <h4 className="text-[22px] lg:text-[26px] font-bold text-[#1A2332] tracking-[-0.01em] mb-3 max-w-md">
                One team, five disciplines, all in orbit around you
              </h4>
              <p className="text-[14px] text-[#5A6B82] leading-relaxed max-w-md">
            ...
            <div className="relative mt-auto px-8 lg:px-10 pb-8 flex flex-wrap gap-2">
              {disciplines.map((label) => (
                <span
                  key={label}
                  className="font-mono text-[10px] uppercase tracking-[0.14em] px-3 py-1.5 rounded-full border border-[#C8D6E5] text-[#1B2B5E] bg-white/80 backdrop-blur-sm"
                >
                  {label}
                </span>
              ))}
          ...
          {/* The Hoardy promise */}
          <div className="lg:col-span-2 rounded-3xl border border-[#C8D6E5] bg-white/80 backdrop-blur-sm p-8 lg:p-10 flex flex-col">
            <p className="eyebrow mb-7">The Hoardy promise</p>
            <ul className="space-y-5">
              {promises.map((promise) => (
                <li key={promise} className="flex items-start gap-3">
                  <span
                    className="mt-0.5 w-5 h-5 rounded-full bg-[#7BB8E8]/20 text-[#1B2B5E] flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                    aria-hidden="true"
                  >
                    ✓
                  </span>
                  <span className="text-[14.5px] text-[#1A2332] leading-relaxed">
                    {promise}
                  </span>
                </li>
              ))}
            ...
            <a
              href="#contact"
              className="mt-auto inline-flex items-center gap-2 pt-8 text-[15px] font-semibold text-[#1B2B5E] hover:text-[#4A82C4] transition-colors duration-200"
            >
              Start your journey
